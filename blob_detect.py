...
def extra_main():
    ...
    for f in range(len(jpg_files)):
        ...
        for c in range(len(corners1)):
          cv2.circle(blob_im1, (corners1[c][0],corners1[c][1]), int(scales1[c]), (255, 0, 0))
          cv2.line(blob_im1, (corners1[c][0],corners1[c][1]), (corners1[c][0]+int(scales1[c]*np.cos(orientations1[c])),corners1[c][1]+int(scales1[c]*np.sin(orientations1[c]))),(255,0,0))
        plt.imsave(img_name+'1_blobs.png', blob_im1)"""
